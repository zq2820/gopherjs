...
func newAnalysisJsx(typesInfo *types.Info, files []*ast.File, fileSet *token.FileSet) *analysisJsx {
	return &analysisJsx{
		typesInfo: typesInfo,
		files:     files,
		fileSet:   fileSet,
...
func (analysis *analysisJsx) run() (errors []error) {
	for _, goFile := range analysis.files {
		if goFile.IsJsx {
			astutil.Apply(goFile, nil, func(cursor *astutil.Cursor) bool {
				analysis.file = goFile
				error := analysis.analysisExpr(cursor)
				if error != nil {
...
func (analysis *analysisJsx) analysisExpr(cursor *astutil.Cursor) *types.Error {
		...
		if _, ok := cursor.Parent().(*ast.GoxExpr); ok {
			if callExpr, ok := node.X.(*ast.CallExpr); ok {
				...
				if selectorExpr, ok := callExpr.Fun.(*ast.SelectorExpr); ok {
					kindObj = analysis.typesInfo.Uses[selectorExpr.Sel]
				} else if ident, ok := callExpr.Fun.(*ast.Ident); ok {
					kindObj = analysis.typesInfo.Uses[ident]
				}
				if kindObj != nil {
					kind := kindObj.Type()
						...
						if tuple != nil {
							if tuple.Len() > 1 {
								error := types.NewError(
									analysis.fileSet,
									node.Rbrace-token.Pos(analysis.fileSet.Base()),
									"GoExpr is only support return one element",
									false,
									0,
									node.Lbrace-token.Pos(analysis.fileSet.Base()),
									node.Rbrace-token.Pos(analysis.fileSet.Base()),
								)
								return &error
							}
				...
				if selectorExpr, ok := node.X.(*ast.SelectorExpr); ok {
					...
					case *ast.Ident:
						kindObj = analysis.typesInfo.Uses[compositeLitType]
					case *ast.SelectorExpr:
						kindObj = analysis.typesInfo.Uses[compositeLitType.Sel]
					case *ast.ArrayType:
	...
	case *ast.GoxExpr:
		...
		tagType := analysis.typesInfo.Uses[id]
		...
		if !isHostElement {
			...
			if typeId, ok := analysis.typesInfo.Uses[id]; ok {
...
func replaceGox(cursor *astutil.Cursor, expr ast.Expr, childType ChildType) {
	if childType == Element {
	...
	} else if childType == NotElementSlice {
		cursor.Replace(&ast.CallExpr{
			Fun: &ast.SelectorExpr{
				X: &ast.Ident{
					Name:    "react",
		...
		cursor.Replace(&ast.CallExpr{
			Fun: &ast.SelectorExpr{
				...
				Sel: &ast.Ident{Name: "Fragment"},
			},
			Args: []ast.Expr{
				expr,
			},
			Ellipsis: cursor.Node().End(),
		})
	}
}
