...
func GenerateNodeModules(projectUrl string, isWatch bool) (quick []string, slow []string) {
	...
	if len(nodeModules) > 0 {
		...
		quick = append(quick, path.Join(projectUrl, fmt.Sprintf("../packages/dist/%s.inc.js", mode)))
		exec.Command("bash", "-c", fmt.Sprintf("rm -f %s", quick[0])).Run()
		if hasNewLib {
			filename := "vendors~index.dev.inc.js"
			if !isWatch {
				filename = "1.prod.inc.js"
			}
			quick = append(quick, path.Join(projectUrl, fmt.Sprintf("../packages/dist/%s", filename)))
			exec.Command("bash", "-c", fmt.Sprintf("rm -f %s", quick[1])).Run()
		} else {
			slow = append(slow, path.Join(projectUrl, fmt.Sprintf("../packages/dist/vendors~index.%s.inc.js", mode)))
		}
		hasNewLib = false
