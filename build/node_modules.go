...
func GenerateNodeModules(projectUrl string, isWatch bool) (quick []string, slow []string) {
	...
	if len(nodeModules) > 0 {
		...
		quick = append(quick, path.Join(projectUrl, fmt.Sprintf("../packages/%s.inc.js", mode)))
		exec.Command("bash", "-c", fmt.Sprintf("rm -f %s", quick[0])).Run()
		if hasNewLib {
			quick = append(quick, path.Join(projectUrl, fmt.Sprintf("../packages/vendors~index.%s.inc.js", mode)))
			exec.Command("bash", "-c", fmt.Sprintf("rm -f %s", quick[1])).Run()
		} else {
			slow = append(slow, path.Join(projectUrl, fmt.Sprintf("../packages/vendors~index.%s.inc.js", mode)))
		}
		hasNewLib = false
		...
		if os.WriteFile(path.Join(projectUrl, "../packages/index.js"), []byte(preload), 0644) == nil {
			...
			if isWatch {
				for {
					...
					if err == nil {
						_, err := os.ReadFile(path.Join(projectUrl, "../packages/vendors~index.dev.inc.js"))
						if err == nil {
							break
						}
					}
